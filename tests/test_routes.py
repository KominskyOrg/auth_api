...
def test_login_invalid_credentials(mock_post, client):
    ...
    mock_post.return_value.json.return_value = {"message": "Invalid credentials"}
...
def test_register_existing_user(mock_post, client):
    ...
    mock_post.return_value.json.return_value = {"message": "User already exists"}
...
def test_reset_password_request_success(mock_post, client):
    mock_post.return_value.status_code = 200
    mock_post.return_value.json.return_value = {"message": "Password reset link sent"}
...
def test_refresh_token_success(mock_post, client):
    ...
    response = client.post(
        "/api/auth/refresh-token", json={"refresh_token": "mock_refresh_token"}
    )
...
def test_refresh_token_invalid(mock_post, client):
    mock_post.return_value.status_code = 401
    mock_post.return_value.json.return_value = {"message": "Invalid refresh token"}
...
def test_deactivate_account_success(mock_post, client):
    ...
    response = client.post(
        "/api/auth/deactivate-account", json={"user_id": "mock_user_id"}
    )
...
def test_deactivate_account_not_found(mock_post, client):
    ...
    response = client.post(
        "/api/auth/deactivate-account", json={"user_id": "nonexistent_user_id"}
    )
...
def test_make_auth_request_unexpected_status_code(mock_post, client):
    ...
    mock_post.return_value.json.return_value = {"message": "Unknown error"}
    response = client.post(
        "/api/auth/login",
        json={"email": "user@example.com", "password": "password"},
    )
